> Tip
>
> 1. 如果你不写@Transactional，那么这个方法是没有事务管理的，Spring不会为你创建一个代理。
> 2. 如果在写了@Transactional，那么这个事务中所有的被调用的方法，都会在事务中，要用事务的传播属性做控制事务应该如何传播。但是调用自己的private方法不会受事务的传播属性的控制，可以用自己的一个实例去调用。
> 3. @Transactional 注解只有作用到 public 方法上事务才生效，不推荐在接口上使用；
> 4. 正确的设置 `@Transactional` 的 `rollbackFor` 和 `propagation` 属性，否则事务可能会回滚失败;
> 5. 被 `@Transactional` 注解的方法所在的类必须被 Spring 管理，否则不生效；
> 6. 底层使用的数据库必须支持事务机制，否则不生效；

## 写在开头

这是一篇关于Spring中，对于事务讲解的比较全面的一篇文章，原文地址：https://javaguide.cn/system-design/framework/spring/spring-transaction.html#transactional-%E6%B3%A8%E8%A7%A3%E4%BD%BF%E7%94%A8%E8%AF%A6%E8%A7%A3。

本片文章中，其中关于事务源码方面的讲解还是过于浅显，还有待补充。另外，关于MySql是如何实现事务的可作为这里的一个扩展方向，另开一篇文章，在MySQL的主题下写出来。
...
![img](Spring事务管理.assets/PlatformTransactionManager.bfc04603-20220503144609703.png)
...
我前段时间在我的[知识星球open in new window](https://www.yuque.com/snailclimb/rpkqw1/pvak2w)分享过：**“为什么我们要用接口？”** 。
...
![img](Spring事务管理.assets/roollbackFor.14ef1320.png)
