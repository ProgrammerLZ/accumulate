...
The original Java memory model was insufficient（不足的）, so the Java memory model was revised in Java 1.5. This version of the Java memory model is still in use in Java today (Java 14+).



## The Internal Java Memory Model

The Java memory model used internally in the JVM divides memory between thread stacks and the heap. This diagram（简图） illustrates the Java memory model from a logic perspective:

![The Java Memory Model From a Logic Perspective](Java%20Memory%20Model/java-memory-model-1.png)
...
![The Java Memory Model showing where local variables and objects are stored in memory.](Java%20Memory%20Model/java-memory-model-2.png)

A local variable may be of a primitive type, in which case it is totally kept on the thread stack.

A local variable may also be a reference to an object. In that case the reference (the local variable) is stored on the thread stack, but the object itself  stored on the heap.

An object may contain methods and these methods may contain local variables. These local variables are also stored on the thread stack, even if the object the method belongs to is stored on the heap.

**An object's member variables are stored on the heap along with the object itself. That is true both when the member variable is of a primitive type（原始类型）, and if it is a reference to an object.**

> 注意：对象的成员变量也存储在堆上，不管这个成员变量是原始类型还是一个对象的引用。
...
![The Java Memory Model showing references from local variables to objects, and from object to other objects.](Java%20Memory%20Model/java-memory-model-3.png)
...
> 注意：只有本地变量，是存储在线程栈上的。成员变量，一律存储在堆上。
...
![Modern hardware memory architecture.](Java%20Memory%20Model/java-memory-model-4.png)

A modern computer often has 2 or more CPUs in it. Some of these CPUs may have multiple cores too. The point is, that on a modern computer with 2 or more CPUs it is possible to have more than one thread running simultaneously（同时）. Each CPU is capable of running one thread at any given time. That means that if your Java application is multithreaded, one thread per CPU may be running simultaneously (concurrently) inside your Java application.

Each CPU contains a set of registers which are essentially（本质上） in-CPU memory. The CPU can perform operations much faster on these registers than it can perform on variables in main memory. That is because the CPU can access these registers much faster than it can access main memory.
...
![The division of thread stack and heap among CPU internal registers, CPU cache and main memory.](Java%20Memory%20Model/java-memory-model-5.png)
...
The following diagram illustrates the sketched（勾勒出） situation. One thread running on the left CPU copies the shared object into its CPU cache, and changes its `count` variable to 2. This change is not visible to other threads running on the right CPU, because the update to `count` has not been flushed back to main memory yet.

![Visibility Issues in the Java Memory Model.](Java%20Memory%20Model/java-memory-model-6.png)

To solve this problem you can use [Java's volatile keyword](http://tutorials.jenkov.com/java-concurrency/volatile.html). T**he `volatile` keyword can make sure that a given variable is read directly from main memory, and always written back to main memory when updated.**

> **valatile**本质上的意义是让共享变量总是能够从主存中读出来并且总是能够及时的写会到主存中去。个人认为这是牺牲了多级缓存和寄存器的速度，来保证多线程情况下的线程安全问题。
>
> **valatile**保证了任何线程都能够读到某个共享变量的最新值。
...
![Race Condition Issues in the Java Memory Model.](Java%20Memory%20Model/java-memory-model-7.png)
...
> **synchronized block**比valatile具有更强的功能。它强制两个线程在同一时间不能够访问相同的代码块，从而避免了竞争问题。而且，他也具备valatile的功能，当线程退出同步代码块的时候，共享变量能够及时的从cpu cache 刷新回主存。