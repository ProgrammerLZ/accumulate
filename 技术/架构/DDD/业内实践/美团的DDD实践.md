...
![服务耦合示意图](美团的DDD实践.assets/8a1f7d38.svg)
...
有一种解决方案，按照演进式设计的理论，让系统的设计随着系统实现的增长而增长。我们不需要作提前设计，就让系统伴随业务成长而演进。这当然是可行的，**<u>敏捷实践中的重构、测试驱动设计及持续集成可以对付各种混乱问题。重构——保持行为不变的代码改善清除了不协调的局部设计，测试驱动设计确保对系统的更改不会导致系统丢失或破坏现有功能，持续集成则为团队提供了同一代码库</u>**。
...
事实上，你可能意识到问题之所在。在解决现实问题时，**<u>我们会将问题映射到脑海中的概念模型，在模型中解决问题，再将解决方案转换为实际的代码</u>**。上述问题在于我们解决了设计到代码之间的重构，但提炼出来的设计模型，并不具有实际的业务含义，这就导致在开发新需求时，其他同学并不能很自然地将业务问题映射到该设计模型。设计似乎变成了重构者的自娱自乐，代码继续腐败，重新重构……无休止的循环。
...
![抽奖ER图](美团的DDD实践.assets/3acb8bbe.svg)
...
**分治** 把问题空间分割为规模更小且易于处理的若干子问题。分割后的问题需要足够小，以便一个人单枪匹马就能够解决他们；其次，必须考虑如何将分割后的各个部分装配为整体。分割得越合理越易于理解，在装配成整体时，所需跟踪的细节也就越少。即更容易设计各部分的协作方式。<u>**评判什么是分治得好，即高内聚低耦合**</u>。
...
<u>**在系统复杂之后，我们都需要用分治来拆解问题。一般有两种方式，技术维度和业务维度。技术维度是类似MVC这样，业务维度则是指按业务领域来划分系统**</u>。
...
以上三种活动在实际开发中是有先后顺序的，但不一定孰先孰后。在我们解决常规套路问题时——也就是简单场景，我们会很自然地往熟悉的分层架构套（先确定系统架构），或者用PHP开发很快（先确定技术架构），在业务不复杂时，这样是合理的。
...
![DDD与微服务关系](美团的DDD实践.assets/b03f17c7.svg)
...
1. <u>**根据需求划分出初步的领域和限界上下文，以及上下文之间的关系；**</u>
2. <u>**进一步分析每个上下文内部，识别出哪些是实体，哪些是值对象；**</u>
3. <u>**对实体、值对象进行关联和聚合，划分出聚合的范畴和聚合根；**</u>
4. <u>**为聚合根设计仓储，并思考实体或值对象的创建方式；**</u>
5. <u>**在工程中实践领域模型，并在实践中检验模型的合理性，倒推模型中不足的地方并重构。**</u>
...
![抽奖平台领域](美团的DDD实践.assets/f9976e81.svg)
...
![C端抽奖领域](美团的DDD实践.assets/9b589fa0.svg)

C端抽奖领域

> 小想法：
>
> 划分限界上下文是比较重要的一个步骤。跟很多中间件的设计类似，比如Tomcat，把整个中间件设计为多个组件，组件是高内聚、低耦合的，通过合适的方式进行沟通。DDD感觉是把一些行业内比较常用的思想进行了论述，最终落实到纸面上并形成知识以供技术人员形成统一的思想，而他所针对的主要人群，我觉得就是业务架构师以及业务开发工程师。而实际上DDD里边所阐述的思想，在中间件开发领域早已开始使用了。
>
> 但是，不得不说的是，在业务开发上，目前阶段以事务脚本代码的方式进行业务的开发是绝对的主流，在中间件领域常用的一些面向对象的思想以及落地设计在这种开发方式中基本是得不到应用。事务脚本代码的开发方式最大的优点就是简单，但是他带来的难维护、难扩展、难测试缺点也是很多的，而这些问题正是面向对象的设计方式所擅长解决的。
>
> DDD提供给了业务开发工程师一种指导思想，让业务开发工程师能够按照面向对象的思想去设计并开发相应的业务功能，从而解决事务脚本代码的开发模式所带来的弊病。而能够熟练的应用DDD思想的工程师，必然能够在面向对象的设计上大展身手。拥有了这样的实践土壤之后，对业务工程师的发展简直是难以估量的，软件领域很多好的面向对象的思想和技巧都能够有用武之地，而非一个个的孤立的、没有得到实践的理论点，被隐藏在不起眼的角落永远都得不到应用。这些过往的好的经验，前人留下来的知识精华并且能够在工作领域进行使用的东西，却因为不知道怎么使用而得不到应用，对我而言，简直是如鲠在喉，难以接受。
...
![上下文映射关系](美团的DDD实践.assets/a694c7a8.svg)
...
![抽奖上下文](美团的DDD实践.assets/725d9171.svg)
...
![数据流转](美团的DDD实践.assets/bb6c4795.svg)
...
![DDD-分层](美团的DDD实践.assets/4480c619.svg)
...
![DDD-系统架构图](美团的DDD实践.assets/db5b1154.svg)
